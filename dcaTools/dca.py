...
def extractTopContacts(dcaFile,numTopContacts,diagIgnore=4):
    ...
    sortedScores = -np.sort(scipy.spatial.distance.squareform(-dca)) # Sort the scores in descending order
    dcaContacts=np.argwhere(dca>sortedScores[numTopContacts])# Extract the contacts
    contactRanks=np.argsort(-dca[dcaContacts[:,0],dcaContacts[:,1]]) # Sort the contacts by score
    dcaContacts = dcaContacts[contactRanks,:] # Sort the contacts by score
