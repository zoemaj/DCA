  ...
  #########################################################
    ...
    for i in range(max_epochs):
        ...
        if validation==False and test==False: #errors[i] is composed of only one list
          avg_train_errors.append(np.mean([np.loadtxt(errors)[i] for errors in ALLerrors]))
          #avg_train_errors_positions.append(np.mean([np.loadtxt(errors_positions[i]) for errors in ALLerrors_positions]))
        elif validation==True and test==False: #errors[i] is composed of two lists
            avg_train_errors.append(np.mean(np.loadtxt(errors)[0][i]) for errors in ALLerrors)
            avg_validation_errors.append(np.mean(np.loadtxt(errors)[1][i]) for errors in ALLerrors)
            #avg_train_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][0])) for errors_positions in ALLerrors_positions)
            #avg_validation_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][1])) for errors_positions in ALLerrors_positions)
            ...
            #avg_train_errors.append(np.mean(np.loadtxt(errors)[0][i]) for errors in ALLerrors)
            #avg_test_errors.append(np.mean(np.loadtxt(errors)[1][i]) for errors in ALLerrors)
            #avg_train_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][0])) for errors_positions in ALLerrors_positions)
            #avg_test_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][1])) for errors_positions in ALLerrors_positions)
        else: #validation and test are true
            avg_train_errors.append(np.mean(np.loadtxt(errors)[0][i]) for errors in ALLerrors)
            avg_validation_errors.append(np.mean(np.loadtxt(errors)[1][i]) for errors in ALLerrors)
            avg_test_errors.append(np.mean(np.loadtxt(errors)[2][i]) for errors in ALLerrors)
            #avg_train_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][0])) for errors_positions in ALLerrors_positions)
            #avg_validation_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][1])) for errors_positions in ALLerrors_positions)
            #avg_test_errors_positions.append(np.mean(np.loadtxt(errors_positions[i][2])) for errors_positions in ALLerrors_positions)
  else:
    print("only one model, the average model is the model")
    average_model=torch.load(ALLmodel[0])
    if validation==False and test==False:
       avg_train_errors=np.loadtxt(ALLerrors)[0]
       #avg_train_errors_positions=np.loadtxt(ALLerrors_positions[1])
    elif validation==True and test==False:
        avg_train_errors=np.loadtxt(ALLerrors)[0][0]
        avg_validation_errors=np.loadtxt(ALLerrors)[0][1]
        #avg_train_errors_positions=np.loadtxt(ALLerrors_positions[0][0])
        #avg_validation_errors_positions=np.loadtxt(ALLerrors_positions[0][1])
    elif validation==False and test==True:
        avg_train_errors=np.loadtxt(ALLerrors)[0][0]
        avg_test_errors=np.loadtxt(ALLerrors)[0][1]
        #avg_train_errors_positions=np.loadtxt(ALLerrors_positions[0][0])
        #avg_test_errors_positions=np.loadtxt(ALLerrors_positions[0][1])
    else: #validation and test are true
        avg_train_errors=np.loadtxt(ALLerrors)[0][0]
        avg_validation_errors=np.loadtxt(ALLerrors)[0][1]
        avg_test_errors=np.loadtxt(ALLerrors)[0][2]
        #avg_train_errors_positions=np.loadtxt(ALLerrors_positions[0][0])
        #avg_validation_errors_positions=np.loadtxt(ALLerrors_positions[0][1])
        #avg_test_errors_positions=np.loadtxt(ALLerrors_positions[0][2])
